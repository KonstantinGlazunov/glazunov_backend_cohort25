...
public class EventsController {
    ...
    public ResponseEntity<List<UserDto>> addParticipantToEvent(
            @PathVariable("event-id") Long eventId,
            @RequestBody UserToEventDto UserData) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(eventService.addParticipantToEvent(eventId, UserData));
    }

    @Operation(summary = "Getting participants of event", description = "only admin available")
    @GetMapping("/{event-id}/participants")
    public ResponseEntity<List<UserDto>> getUserOfEvent(@PathVariable("event-id") Long eventId){
